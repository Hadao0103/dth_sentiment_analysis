...
# clean text v2
    ...
    def predict_sentiment(restaurant_name):
        ...
        X_new_cv = count_model.transform(X_new).toarray()

        # predict data from loaded model
        y_pred_prob = model1.predict(X_new_cv)
        y_pred_class = y_pred_prob >= 0.5

        # save result
        df_restaurant['Predict Class'] = y_pred_prob
        df_restaurant['Positive Review'] = y_pred_class

        result = df_restaurant.drop('restaurant', axis=1)
...
#----------------------------------------------------------------------------------------------------
    ...
    if choice3_input == 'Choose restaurant':
        ...
        # Load model
        filename1 ='model/naive_model.pkl'
        with open(filename1, 'rb') as f:
            model1 =pickle.load(f)

        # Đọc model countvectorizer
        filename2 ='model/count_model.pkl'
        ...
        if submit_button:
            ...
            n_review_postitive = sum(1 for x in result['Positive Review'] if x == True)
            n_review_negative = sum(1 for x in result['Positive Review'] if x == False)
        ...
        if choice3_model == 'Model 1':
            ...
            X_new_cv = count_model.transform(X_new1).toarray()

            y_pred_newtest1 = model1.predict(X_new_cv)
            yhat1 = y_pred_newtest1
        ...
        # content
            if choice3_model == 'Model 1':  
                ...
                # prediction
                uploaded_data['text_cleaned'] = uploaded_data.review_text.apply(lambda x: clean_text(str(x)))
                X_new = uploaded_data['text_cleaned']
                X_new_cv = count_model.transform(X_new).toarray()
                yhat = model1.predict(X_new_cv)
                ...
                result_df.loc[result_df['sentiment']==1, 'sentiment'] = 'positive'
                result_df.loc[result_df['sentiment']==0, 'sentiment'] = 'negative'                

                # show results
                st.write("<h5 style='text-align: left; color: Black;'>Input information:</h5>", unsafe_allow_html=True)
                st.write("1. Model name: Model 1")
...
# 4. Conclusion and Next steps
    ...
    st.write('''First, We should focus on handling negative feedback to avoid losing existing customers.
                We will use this model to evaluate customer sentiment,
                not just based on their rating because the fact shows that there are bad responses but the rating does not show all.''')
    st.write('''From negative reviews,
                restaurants need to focus on solving negative issues
                that customers feedback such as food quality, space, service, waiting time, delivery time, ...''')
    st.write('''Along with that will conduct gratitude/ push sales programs to attract new customers 
                and also from new better experiences,
                customers who rate better on the app will also be a source of attraction. guest.''')


    st.header('NEXT STEP')
    st.write('''**For high positive feedbacks**, reaching 0.9-1 on probability predicts satisfaction 
                => We could award rewards according to service quality to boost employee morale
                and can be accompanied by internal activities to convey energy. positive feedback to the remaining employees.''')
    st.write('''**For negative feedbacks**,
                it is necessary to trace the same cause of the problem.
                We can check the camera and double check the staff that day to see at that time what is wrong with the customer's meal,
                how the customer reacts and the staff's attitude in that case. How did you handle it?''')
    st.write('+ If the treatment plan is not good or there is no treatment plan: retrain staff.')    
    st.write('+ If you have a plan to handle it and feel it is reasonable, you can use it as evidence to discuss with other employees.')            
